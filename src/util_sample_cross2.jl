...
"""
function gen_sim_sols_probs(; tools, psets, solver = solver)
    ...
    problemupdater = get_concrete(first(tools).problemupdater)) -> let tools = tools,
        ...
        kwargs...) -> begin
            ...
            map(1:n_indiv) do i_indiv
                ...
                problem_opt = tools[i_indiv].problem # no need to copy, in update_statepar 
                ...
                sol = with_logger(fLogger) do
                    sol = solve(problem_opt, solver; maxiters = 1e4, kwargs...)
                end
                (; sol, problem_opt)
            end # map 1:n_indiv
        end # function  

...
"""
    function gen_model_cross(;
            ...
            scenario, psets, solver, T_streams=missing)
...
When changing stream temperatures, using ComponentVector `T_Streams`, the model 
needs to be regenerated in order to capture all in used arguments in closure.    
"""
function gen_model_cross(;
        inv_case::AbstractCrossInversionCase, tools, priors_pop, sim_sols_probs,
        scenario, psets, solver, r=1.0, w_streams=missing, stream_nums=missing)
    ...
    obs_target = map(obs) do obs_indiv
        map(obs_site_stream -> obs_site_stream.obs, obs_indiv)
    end
    streams_u = reduce(union, (keys(s) for s in obs))
    if (ismissing(w_streams)) 
        w_streams = CA.ComponentVector(ones(length(streams_u)), CA.Axis(streams_u))
    end
    T_streams = compute_T_streams_obs(obs, r, w_streams)
    # compute temperated uncertainty and add it as component obs_uncT to each stream
    dtypes = (;
    zip(streams_u,
        (get_case_obs_uncertainty_dist_type(inv_case, s; scenario)
        for s in streams_u))...)
    obsT = map(enumerate(obs)) do (i_indiv, obs_indiv)
        streams = keys(obs_indiv)
        v= map(streams) do stream
            Ti = T_streams[i_indiv][stream]
            obs_s = obs_indiv[stream]
            (stream, (;obs_s..., obs_uncT = temper_unc(dtypes[stream], obs_s.obs, obs_s.obs_unc, Ti)))
        end
        (;v...)
    end
    if ismissing(stream_nums)
        stream_nums = (;
            zip(streams_u,
                Symbolics.scalarize.(getindex.(Ref(sys_num_dict), streams_u)))...)
    else
        # check that all streams are in stream_nums
        mis = (s for s in streams_u if s ∉ keys(stream_nums))
        isempty(mis) || error(
            "Expected mapping of stream indices to solution indices, stream_nums, " *
            "to have entries for each stream, but missing following streams: " * 
            join(mis, ","))
    end
    # use Multivariate Distribution for all time points?
    #   yes, if specified Multivariate but a scalar observation at time
    is_multivarite_dist_stream = map(Base.Fix2(<:,MultivariateDistribution), dtypes) 
    is_scalar_obs_stream  = (;map(keys(is_multivarite_dist_stream)) do stream
        obs_indiv_with_stream = obs[findfirst(obs_indiv -> stream ∈ keys(obs_indiv), obs)]
        (stream, !(eltype(obs_indiv_with_stream[stream].obs) <: AbstractVector))
    end...)
    is_collect_times = map((mv, sc) -> mv & sc, is_multivarite_dist_stream , is_scalar_obs_stream)
    #
    gen_tmodel_cross = let pdf_deficit = Exponential(0.001),
        ...
        obsT = obsT,
        dtypes = dtypes,
        #saveat = union(map_keys(stream -> stream.t, obsT)),
        stream_nums = stream_nums,
        is_collect_times = is_collect_times
        ...
        Turing.@model function tmodel_cross(obs_target, ::Type{T} = Float64) where {T}
            ...
            for i_indiv in 1:n_indiv
                ...
                streams_i = keys(obsT[i_indiv])
                #stream = first(streams_i)
                for stream in streams_i
                    obss = obsT[i_indiv][stream]
                    pred = sol(obss.t; idxs = stream_nums[stream]).u
                    #(i,t) = first(enumerate(obss.t))
                    if is_collect_times[stream]
                        uncT = obss.obs_uncT 
                        dist_pred = fit_mean_Σ(dtypes[stream], pred, uncT)
                        obs_target[i_indiv][stream] ~ dist_pred
                        #obs_target[i_indiv][stream], rand(dist_pred,5)
                        else
                        for (i, t) in enumerate(obss.t)
                            pred_t = pred[i]
                            uncT = obss.obs_uncT[i] 
                            dist_pred = fit_mean_Σ(dtypes[stream], pred_t, uncT)
                            #tmp = rand(dist_pred)
                            obs_target[i_indiv][stream][i] ~ dist_pred
                            #obss.obs[i], rand(dist_pred,5)
                        end
                    end
                end
...
function compute_T_streams_obs(obs, r, w_streams)
    map(obs) do obs_indiv
        streams = keys(obs_indiv)
        T_indiv = (; map(streams) do s
            cnt_obs = count_finite_obs(obs_indiv[s].obs)
            T_s = compute_T_streams(cnt_obs, r, w_streams[s])
            (s, T_s)
        end...)
    end
end
...
count_finite_obs(obs::AbstractVector{<:Number}) = sum(skipmissing(isfinite.(obs)))
function count_finite_obs(obs::AbstractVector{<:AbstractVector}) 
    reduce(+, map(obs) do obs_i 
        !any(ismissing.(obs_i)) && all(isfinite.(skipmissing(obs_i)))
    end; init=0)
end

...
_setuptools = (indiv_id, u0, p) -> setup_tools_scenario(indiv_id; inv_case, scenario, system, u0, p, 
    keys_indiv = component_keys.indiv);
...
s1 = CA.ComponentMatrix(Array(chn[:,1:length(sample0),1]), CA.FlatAxis(), first(CA.getaxes(sample0)))
s1[:,:fixed][:,:sv₊p]